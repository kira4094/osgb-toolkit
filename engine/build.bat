@echo off
REM ============================================================
REM build.bat — 编译 osgb_engine.exe (官方 OSG 3.6.5 + qslim GH)
REM ============================================================
setlocal
...
set "EIGENINC=D:\WindowsOS\Temp\eigen-3.4.0"

set "CL_EXE=%MSVC%\bin\Hostx64\x64\cl.exe"
set "INCLUDE=%MSVC%\include;%SDK%\Include\%SDKVER%\ucrt;%SDK%\Include\%SDKVER%\um;%SDK%\Include\%SDKVER%\shared;%OSGINC%;%EIGENINC%"
set "LIB=%MSVC%\lib\x64;%SDK%\Lib\%SDKVER%\ucrt\x64;%SDK%\Lib\%SDKVER%\um\x64;%OSGLIB%"

echo === Compiling osgb_engine.exe ===
"%CL_EXE%" /nologo /EHsc /O2 /std:c++20 /D NOMINMAX /D WIN32_LEAN_AND_MEAN /D NDEBUG /utf-8 ^
  /I "%OSGINC%" /I "%EIGENINC%" /I "%ENGINEDIR%" /I "%ENGINEDIR%\gh" ^
  "%ENGINEDIR%\osgb_engine.cpp" ^
  /link /LIBPATH:"%OSGLIB%" osg.lib osgDB.lib osgUtil.lib OpenThreads.lib ^
  /OUT:"%ENGINEDIR%\osgb_engine.exe"
